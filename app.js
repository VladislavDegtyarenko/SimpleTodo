...
const generalSortFunction = (sortBy, isReverse) => {
   if (!localStorage[taskListName]) return;
   let taskList = JSON.parse(localStorage[taskListName]);

   const sortingParameter = (function () {
      if (sortBy === "sortByName") return "taskName";
      if (sortBy === "sortByPriority") return "taskPriority";
      if (sortBy === "sortByDeadline") return "taskDate";
   })();
   console.log(sortingParameter);

   taskList.sort((x, y, sortingParameter, isReverse) => {
      console.log(sortingParameter);
      console.log(a);
      console.log(b);
...
const sortByPriority = () => {
   ...
   sortByPriorityBtn.hasAttribute("reverse") ? taskList.sort(comparePriorityReverse) : taskList.sort(comparePriority);
   sortByPriorityBtn.toggleAttribute("reverse");
...
listButtons.onclick = (e) => {
   if (e.target.matches("button")) {
      if (e.target.classList.contains("sortBtn")) {
         ...
         generalSortFunction(sortBy, isReverse);
      }
   }
};
